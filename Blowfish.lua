...
class. Blowfish
{
	Blowfish	= function( k )
		...
		this.key	= k:len() > 56 and k:sub( 1, 56 ) or k;
		
		local j		= 0;
		
		for i = 0, 17 do
			local b0	= this.key:byte( ( j % this.key:len() ) + 1 );
			local b1	= this.key:byte( ( ( j + 1 ) % this.key:len() ) + 1 );
			local b2	= this.key:byte( ( ( j + 2 ) % this.key:len() ) + 1 );
			local b3	= this.key:byte( ( ( j + 3 ) % this.key:len() ) + 1 );
			
			local d		= ( ( b0 * 256 + b1 ) * 256 + b2 ) * 256 + b3;
			
			this.bf_P[ i ] = bitXor( this.bf_P[ i ], d );
	...
	Encrypt		= function( t )
		...
		for i = 1, t:len(), 16 do
			this.xr_par	= this.wordunescape( t:sub( i, i + 7 ) );
			this.xl_par	= this.wordunescape( t:sub( i + 8, i + 15 ) );
			
			this.encipher();
			
			result = result .. this.wordescape( this.xr_par ) .. this.wordescape( this.xl_par );
		end
	...
	Decrypt		= function( t )
		...
		for i = 1, t:len(), 16 do
			...
			-- print( this.xr_par, this.xl_par );
			
			this.decipher();
			
			local a = this.wordescape( this.xr_par );
			local b = this.wordescape( this.xl_par );
			...
			bitXor(
				(
					(
						bitXor(
							(
								this.bf_S0[ this.wordbyte0( b ) ]
								+
								this.bf_S1[ this.wordbyte1( b ) ]
							),
							this.bf_S2[ this.wordbyte2( b ) ]
						)
					)
					+ this.bf_S3[ this.wordbyte3( b ) ]
				),
				this.bf_P[ n ]
			)
		);
	...
	encipher	= function()
		local Xl	= this.xl_par;
		local Xr	= this.xr_par;
		
		Xl	= bitXor( Xl, this.bf_P[ 0 ] );
		
		for i = 1, 16, 2 do
			Xr	= this.round( Xr, Xl, i );
			Xl	= this.round( Xl, Xr, i + 1 );
		end
		
		Xr	= bitXor( Xr, this.bf_P[ 17 ] );

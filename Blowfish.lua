...
function Blowfish:Blowfish( k )
	if type( k ) ~= 'string' then
		error( "Type mismatch in parameter #1 (wanted 'string', got '" .. type( k ) .. "')", 2 );
	end
	
	if k:len() == 0 then
		error( "Bad argument #1, string is empty", 2 );
	end
	
	self.bf_P	= 
	{
	[0]=0x243F6A88,0x85A308D3,0x13198A2E,0x03707344,0xA4093822,0x299F31D0,0x082EFA98,0xEC4E6C89,0x452821E6,
		0x38D01377,0xBE5466CF,0x34E90C6C,0xC0AC29B7,0xC97C50DD,0x3F84D5B5,0xB5470917,0x9216D5D9,0x8979FB1B
	};
	
	self.bf_S0	= 
	{  
	[0]=0xD1310BA6,0x98DFB5AC,0x2FFD72DB,0xD01ADFB7,0xB8E1AFED,0x6A267E96,0xBA7C9045,0xF12C7F99,0x24A19947,
	...
	self.bf_S1	= 
	{
	[0]=0x4B7A70E9,0xB5B32944,0xDB75092E,0xC4192623,0xAD6EA6B0,0x49A7DF7D,
	...
	self.bf_S2	= 
	{ 
	[0]=0xE93D5A68,0x948140F7,0xF64C261C,0x94692934,0x411520F7,0x7602D4F7,
	...
	self.bf_S3	= 
	{
	[0]=0x3A39CE37,0xD3FAF5CF,0xABC27737,0x5AC52D1B,0x5CB0679E,0x4FA33742,
	...
	self.key	= k:len() > 56 and k:sub( 1, 56 ) or k;
	
	local j		= 0;
	
	for i = 0, 17 do
		local b0	= self.key:byte( ( j % self.key:len() ) + 1 );
		local b1	= self.key:byte( ( ( j + 1 ) % self.key:len() ) + 1 );
		local b2	= self.key:byte( ( ( j + 2 ) % self.key:len() ) + 1 );
		local b3	= self.key:byte( ( ( j + 3 ) % self.key:len() ) + 1 );
		
		local d		= ( ( b0 * 256 + b1 ) * 256 + b2 ) * 256 + b3;
		
		self.bf_P[ i ] = bitXor( self.bf_P[ i ], d );
...
function Blowfish:escape( value )
	local result = '';
	
	for i = 1, value:len() do
		local char	= value:byte( i );
		local t1	= math.floor( char / 16 );
		local t2	= char % 16;
		...
		result = result .. string.char( t1 ) .. string.char( t2 );
	end
	
	return result;
end

function Blowfish:unescape( value )
	local result = '';
	
	for i = 1, value:len(), 2 do
		local t1	= value:byte( i );
		local t2	= value:byte( i + 1 );
...
function Blowfish:wordbyte0( w )
	if not w then error( "attempt to perform arithmetic on local 'w' (a nil value)", 2 ); end
	
	return math.floor( math.floor( math.floor( w / 256 ) / 256 ) / 256 ) % 256;
end

function Blowfish:wordbyte1( w )
	if not w then error( "attempt to perform arithmetic on local 'w' (a nil value)", 2 ); end
	
	return math.floor( math.floor( w / 256 ) / 256 ) % 256;
end

function Blowfish:wordbyte2( w )
	if not w then error( "attempt to perform arithmetic on local 'w' (a nil value)", 2 ); end
	
	return math.floor( w / 256 ) % 256;
end

function Blowfish:wordbyte3( w )
	if not w then error( "attempt to perform arithmetic on local 'w' (a nil value)", 2 ); end
	
	return w % 256;
end

function Blowfish:Encrypt( t )
	local t = self:escape( t );
	local i = 0;
	...
	for i = 1, t:len(), 16 do
		self.xr_par	= self:wordunescape( t:sub( i, i + 7 ) );
		self.xl_par	= self:wordunescape( t:sub( i + 8, i + 15 ) );
		
		self:encipher();
		
		result = result .. self:wordescape( self.xr_par ) .. self:wordescape( self.xl_par );
	end
	
	return result;
end

function Blowfish:Decrypt( t )
	t = t:upper();
	...
	for i = 1, t:len(), 16 do
		...
		-- print( self.xr_par, self.xl_par );
		
		self:decipher();
		
		local a = self:wordescape( self.xr_par );
		local b = self:wordescape( self.xl_par );
...
function Blowfish:wordescape( w )
	local result = '';
	...
	for i = 3, 0, -1 do
		...
		-- print( string.char( t1 ), t1, string.char( t2 ), t2 );
		
		local c1, c2 = string.char( t1 ), string.char( t2 );
		
		-- if c1 == '0' and c2 == '0' then
			-- print( c1, c2, t1, t2, t11, t22 );
...
function Blowfish:wordunescape( value )
	local r = 0;
		...
		bitXor(
			(
				(
					bitXor(
						(
							self.bf_S0[ self:wordbyte0( b ) ]
							+
							self.bf_S1[ self:wordbyte1( b ) ]
						),
						self.bf_S2[ self:wordbyte2( b ) ]
					)
				)
				+ self.bf_S3[ self:wordbyte3( b ) ]
			),
			self.bf_P[ n ]
		)
	);
...
function Blowfish:encipher()
	local Xl	= self.xl_par;
	local Xr	= self.xr_par;
	
	Xl	= bitXor( Xl, self.bf_P[ 0 ] );
	
	for i = 1, 16, 2 do
		Xr	= self:round( Xr, Xl, i );
		Xl	= self:round( Xl, Xr, i + 1 );
	end
	
	Xr	= bitXor( Xr, self.bf_P[ 17 ] );
	
	self.xl_par	= Xr;
	self.xr_par	= Xl;
end

function Blowfish:decipher()
	local Xl	= self.xl_par;
	local Xr	= self.xr_par;
	
	Xl	= bitXor( Xl, self.bf_P[ 17 ] );
